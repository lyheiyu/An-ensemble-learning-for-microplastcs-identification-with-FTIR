...
def parseData2(fileName, begin, end):
    ...
    return polymerName, waveLength, intensity, polymerNameData,transfromerData
# -*- Coding: UTF-8 -*-
...
def get_data():
    polymerName, waveLength, intensity, polymerID, transfromerData = parseData2('D4_4_publication5.csv', 2, 1763)
    # from pandas import read_csv
    # series = read_csv('daily-min-temperatures.csv', header=0, index_col=0, parse_dates=True, squeeze=True)
    print(transfromerData)
    polymerID = np.array(polymerID)
    polymerID = polymerID.reshape(964, 1)
    print(polymerID.shape)
    intensity = scaler.fit_transform(intensity)
    print(intensity.shape)
    # np.append([[1, 2, 3], [4, 5, 6]], [[7, 8, 9]], axis=0)
    dataTrain = np.concatenate((intensity, polymerID), axis=1)
    #dataTrain = torch.FloatTensor(dataTrain)
    print(dataTrain)
    dataForTrain, dataForTest = train_test_split(dataTrain, test_size=0.3, random_state=1)

    train_data, test_data = train_test_split(dataTrain, train_size=0.7, random_state=1)
    ...
    print(train_data)
    #est_data = test_data[:1761]

    train_padding = get_padding_mask(train_data1)
    test_padding = get_padding_mask(test_data1)

    train_seq = torch.from_numpy(train_data[:, 0:1760])
    train_label=train_data[:,1761]
    train_label = train_label.reshape(674, 1)
    train_label = torch.from_numpy(train_label)

    test_seq = torch.from_numpy(test_data[:, 0:1760])
    test_label = torch.from_numpy(test_data[:, 1761])
    print(train_seq)
    train_padding = torch.from_numpy(np.array(train_padding))
    print(train_padding)
    test_padding = torch.from_numpy(np.array(test_padding))
