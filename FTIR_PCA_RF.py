...
if __name__ == '__main__':
    ...
    finalData, reconMat, Vector = main(104)
    ...
    index=[]
    recallScore=[]
    model = RandomForestClassifier(n_estimators=74, oob_score=True,
    ...
    K_report.to_csv('PCA_RF_report.csv')
    #t.to_csv('Knn_report5.csv')
    print(t)
    KNN_Confusion_matrix=pd.DataFrame(cm)
    KNN_Confusion_matrix.to_csv('PCA_RF_CM.csv')
    PN=getPN('D4_4_publication5.csv')
    plot_confusion_matrix(cm,PN,'PCA-RF')
